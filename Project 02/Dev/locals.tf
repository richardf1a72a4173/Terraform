...
locals {
  ...
  app_hostname = var.http_hostname == "" ? "${var.project_tag}-${local.env}-${var.country_tag}-001.${data.azurerm_dns_zone.devdns.name}" : var.http_hostname
  ...
  dns_a_name = "${var.project_tag}-${local.env}-${var.country_tag}-001"

  sql_priv_name     = "pe-mysql${local.resource_template}001"
  sql_priv_ser_name = "psc-mysql${local.resource_template}001"
