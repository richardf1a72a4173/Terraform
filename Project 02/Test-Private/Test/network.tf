...
resource "azurerm_subnet" "app-gw" {
  ...
  resource_group_name  = azurerm_resource_group.rg.name
  name                 = "${module.naming.subnet.name}-001"
...
resource "azurerm_subnet" "app" {
  virtual_network_name = azurerm_virtual_network.vnet.name
  resource_group_name  = azurerm_resource_group.rg.name
  name                 = "${module.naming.subnet.name}-002"
...
resource "azurerm_subnet" "pe" {
  virtual_network_name = azurerm_virtual_network.vnet.name
  resource_group_name  = azurerm_resource_group.rg.name
  name                 = "${module.naming.subnet.name}-003"
...
resource "azurerm_subnet" "sql" {
  virtual_network_name = azurerm_virtual_network.vnet.name
  resource_group_name  = azurerm_resource_group.rg.name
  name                 = "${module.naming.subnet.name}-004"
...
resource "azurerm_application_security_group" "asg" {
  ...
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
  name                = "${module.naming.application_security_group.name}-00${count.index + 1}"
...
resource "azurerm_network_security_rule" "appgw-clients" {
  ...
  destination_address_prefixes = concat(azurerm_subnet.app-gw.address_prefixes, ["${data.azurerm_public_ip.pip.ip_address}/32"])
